...
def board_setup(board):
    template_row = ["O", color.BLUE + "O", "O", "O", "O", "O", "O", "O", "O", "O", "O" + color.END] # Empty rows
    empty_row = [ ]
    alpha = 65
...
carrier.position("Carrier", 5)
carrier.placing("Carrier", 5)

battle = place_ship("Battleship", 4)
battle.position("Battleship", 4)
battle.placing("Battleship", 4)

destroyer = place_ship("Destroyer", 3)
destroyer.position("Destroyer", 3)
destroyer.placing("Destroyer", 3)

submarine = place_ship("Submarine", 3)
submarine.position("Submarine", 3)
submarine.placing("Submarine", 3)

paddle = place_ship("Paddle Boat", 2)
paddle.position("Paddle Boat", 2)
paddle.placing("Paddle Boat", 2)
...
def start_screen():
    ...
    while True:
        ...
        if user_input1 in start_inputs:
            ...
            print("\n")
            print("Battleship is a game of strategy and deduction. In the single-player version, you will play against the computer.\n\n" \
                  ...
                  "Submarine: 3 holes\n" \
                  "Paddle Boat: 2 holes\n" \
                  "The computer will place the ships in the ocean, either horizontally or vertically - but not diagonally. Ships will not overlap each other\n" \
                  "or overhang the 10x10 square.\n" \
                  "\n" \
                  "Your board will represent your shots. In each turn, you will pick a target. Rows are represented by letters A-J and columns by numbers 1-10.\n" \
                  "Each shot must be in the format LetterNumber: eg. A1, H10 etc.\n" \
                  "If your shot hit a ship, your board will be updated with a red X on your board. If you missed, a white X will be placed instead.\n" \
                  "You will not be told what shipped you have hit, but you will be told when a ship has been sunk.\n" \
                  "After each turn, you will be told how many turns you have played, how many ships are still in play and the status of each ship (still in play or sunk).\n" \
                  "You will win the game when all 5 enemy ships have been sunk.\n" \
                  "If you want to quit mid-game, enter Quit, QUIT, Q, q, Exit, EXIT, E, or e when prompted for your shot.\n")
